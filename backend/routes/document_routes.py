...
async def list_documents(
    ...
    if role == "readonly":
        query["status"] = {"$in": ["active", "review_due", "review_overdue"]}
    elif role == "reviewer":
        query["$or"] = [
...
async def get_document(doc_id: str, request: Request):
    ...
    role = current_user["role"]
    uid = current_user["id"]
    if role == "readonly" and doc["status"] not in ["active", "review_due", "review_overdue"]:
...
async def approve_action(doc_id: str, request: Request, body: ApproveActionRequest):
    ...
    uid = current_user["id"]
    if doc["approver_id"] != uid and current_user["role"] != "admin":
