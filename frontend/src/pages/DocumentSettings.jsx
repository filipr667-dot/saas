...
export default function DocumentSettings() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const res = await api.get("/settings/doc-types");
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-foreground tracking-tight">Document Settings</h1>
        <p className="text-sm text-muted-foreground mt-0.5">Configure document types and review periods</p>
      </div>
      ...
      <div className="border border-border rounded-md bg-card">
        ...
        <div className="divide-y divide-border">
          {loading ? (
            ...
            <div key={dt.id} className="flex items-center gap-4 px-5 py-3 hover:bg-muted/30 transition-colors">
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs text-muted-foreground border border-border rounded px-1.5 py-0.5">{dt.prefix}</span>
              ...
              <div className="flex gap-1">
                <button onClick={() => openEdit(dt)} title={`Edit ${dt.name}`}
          ...
          <div className="relative bg-card border border-border rounded-md p-6 w-full max-w-sm z-10 shadow-xl">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              {modal === "create" && (
                <div>
                  ...
                  <input type="text" value={form.prefix}
                    onChange={(e) => setForm({ ...form, prefix: e.target.value.toUpperCase() })} required maxLength={6}
              ...
              <div>
                ...
                <input type="number" min={1} max={120} value={form.review_period_months}
                  onChange={(e) => setForm({ ...form, review_period_months: e.target.value })} required
                  className="w-full px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-1 focus:ring-ring" />
